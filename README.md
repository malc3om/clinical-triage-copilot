...
The copilot's reasoning is optionally generated by a frontier open LLM served
by **[Featherless.ai](https://featherless.ai)** — an OpenAI-compatible,
serverless API (no GPUs to manage). Set one key and the LLM policy activates:

```bash
# The API key is a bearer token sent on every request:
...
|:--------|:--------|:--------|
| `FEATHERLESS_API_KEY` | — | Enables the LLM policy (bearer token) |
| `FEATHERLESS_BASE_URL` | `https://api.featherless.ai/v1` | OpenAI-compatible endpoint |
| `FEATHERLESS_MODEL` | `deepseek-ai/DeepSeek-V3.2` | Model id, used verbatim in the API call |

**Provider priority:** `FEATHERLESS_API_KEY` → `GEMINI_API_KEY` (Google Gemini
fallback) → `HF_TOKEN` (Hugging Face router) → **deterministic heuristic
